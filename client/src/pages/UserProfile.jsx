    ...
    try {
      ...
      if (response.ok) {
        ...
        setUserData(prevData => 
          prevData.map(post => 
            post._id === postId ? { ...post, like: updatedPost.upload.like } : post
          )
        );
      }
      else {
    ...
    <>
      ...
      <div className="profile-tabs flex justify-around border-b border-gray-600">
        <span
          ...
          className={`cursor-pointer px-4 py-2 font-semibold ${page === "posts" ? "text-yellow-500 border-b-2 border-yellow-500" : "text-gray-200"
            }`}
        >
          POST
        </span>
        <span
          ...
          className={`cursor-pointer px-4 py-2 font-semibold ${page === "photos" ? "text-yellow-500 border-b-2 border-yellow-500" : "text-gray-200"
            }`}
        >
          PHOTOS
        </span>
        <span
          ...
          className={`cursor-pointer px-4 py-2 font-semibold ${page === "videos" ? "text-yellow-500 border-b-2 border-yellow-500" : "text-gray-200"
            }`}
        >
          VIDEOS
        </span>
      ...
      {page === "posts" && <UserPosts userData={userData} handleStarCount={handleStarCount} />}
      {page === "photos" && <UserPhotos userUploadedPictures={userUploadedPictures}/>}
      ...
      {userData.map((post, index) => (
        <div className="post" key={index}>
          ...
          <div className="post-footer" >
            <span>{post.like.length ? post.like.length : 0 }</span> <span className="star" onClick={() => handleStarCount(post._id)}>★ Star</span>
          </div>
        </div>
      ))}
    </main>
...
const UserPhotos = ({ userUploadedPictures }) => {
  ...
  const prevImage = (e) => {
    e.stopPropagation();
    setCurrentIdx((prev) => (prev === 0 ? userUploadedPictures.length - 1 : prev - 1));
  };

  const nextImage = (e) => {
    e.stopPropagation();
    setCurrentIdx((prev) => (prev === userUploadedPictures.length - 1 ? 0 : prev + 1));
  };
